...
from src.tokenizer.basic_tokenizer import BasicTokenizer, CONTEXT_SIZE

# Global variables
TOTAL_SIZE = 1e5 # 10MB * 65 = 650 MB
...
if __name__ == "__main__":
    ...
    for split, p in splits:
        ...
        encodings_path = os.path.join(os.path.dirname(
             os.path.abspath(__file__)), f'data/{split}_ids.bin'
             )
        ...
        scores_path = os.path.join(
             os.path.dirname(os.path.abspath(__file__)), f'data/{split}_percentages.bin'
             )
        print(scores_path)
        # Creating memory mapped array at filepath
        ...
        for game in data['train']:
            # Create mini batches
            batch_ids = game['ids']
            batch_bins = game['win_percentages']
            ...
            percentages_arr[write_idx: write_idx + B] = batch_bins[:B]
